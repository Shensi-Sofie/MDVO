...
RESULT_PATH_PREFIX = '../results2/ddqn_2000ep_'
MODEL_PATH = '../models/'
...
class VehicleAgent(object):
    def __init__(self, index):
        ...
        self.agent = DDQN(actions_num, self.index)
    ...
    def generate_task(self):
        self.data_size = round(np.random.poisson(lam=4))
    ...
    def get_action(self, obs):
        self.action = self.agent.action(obs)
...
class OffloadingRunner(object):
    def __init__(self, agents_num):
        ...
        for j in range(1, self.mec_num + 1):
            self.mec_agents.append(MecAgent(j))
        for i in range(self.agents_num):
            self.agents.append(VehicleAgent(i))
    ...
    def store_trajectories(self, rewards, delays, local_waits, edge_delays, local_datas):
        for k, i in zip(self.agents, range(len(self.agents))):
            if len(k.buffer) >= REPLAY_BUFFER_SIZE:
                k.buffer.pop(0)
            k.buffer.append([rewards[i], delays[i], local_waits[i],
                             edge_delays[i], local_datas[i]])
...
if __name__ == '__main__':
    ...
    for ep in range(MAX_EP):
        ...
        for t in range(EP_LEN):  # 641个时间步，每个时间步是0.1s
            ...
            # 保存轨迹数据
            runner.store_trajectories(rewards, delays, local_waits, edge_delays, local_datas)
            for k, i in zip(runner.agents, range(len(runner.agents))):
                k.agent.perceive(obs[i], actions[i], rewards[i], new_obs[i])
            # 更新observation
            obs = new_obs
            counter += 1
            # 每运行batch_size步, 训练一次
            if ep > 5 and counter >= BATCH_SIZE:
                if counter % BATCH_SIZE == 0:
                    for m in runner.agents:
                        m.agent.train_Q_network()
            if counter % REPLACE_TARGET_FREQ == 0:
                for n in runner.agents:
                    n.agent.update_target_q_network()
        # 保存各项评价指标
